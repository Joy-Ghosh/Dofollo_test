...
function MetricCard({ metric, i, inView }: { metric: any; i: number; inView: boolean }) {
    ...
    return (
        <div className="relative p-8 bg-white rounded-2xl border border-[#0A2E22]/10 shadow-sm hover:shadow-lg transition-all text-center group bg-opacity-80 hover:bg-white overflow-hidden">
            {/* Progress ring (decorative arc) */}
            <div className="absolute -top-4 -right-4 opacity-10">
                <svg width="100" height="100" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="40" fill="none" stroke="#045C4E" strokeWidth="2" />
                    <circle
                        cx="50" cy="50" r="40" fill="none" stroke="#045C4E" strokeWidth="3"
            ...
            <div
                className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-[#0A2E22] mb-2 relative z-10 tabular-nums group-hover:scale-110 transition-transform duration-300"
            ...
            <div className="text-sm font-bold tracking-wide uppercase text-[#045C4E] relative z-10">{metric.label}</div>
        </div>
    );
}
...
export default function ImpactSection() {
    ...
    return (
        <section className="py-24 md:py-32 bg-[#F0FFF8] text-[#0A2E22]">
            <div className="container mx-auto">
                <ScrollReveal variant="fade-up" className="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <h2 className="text-3xl lg:text-5xl font-extrabold mb-6 tracking-tight text-[#0A2E22]">
                        {impact_section.heading_first} <span className="text-[#045C4E]">{impact_section.heading_highlight}</span>
                    </h2>
                </ScrollReveal>
